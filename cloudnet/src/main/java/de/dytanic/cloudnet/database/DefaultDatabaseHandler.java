...
public final class DefaultDatabaseHandler implements IDatabaseHandler {
  ...
  public void handleInsert(Database database, String key, JsonDocument document) {
    ...
    if (!database.isSynced()) {
      CloudNet.getInstance().getClusterNodeServerProvider().sendPacket(
        new PacketServerLocalDatabase(PacketServerLocalDatabase.OperationType.INSERT, database.getName(), key, document)
      );
    }
  }
  ...
  public void handleUpdate(Database database, String key, JsonDocument document) {
    ...
    if (!database.isSynced()) {
      CloudNet.getInstance().getClusterNodeServerProvider().sendPacket(
        new PacketServerLocalDatabase(PacketServerLocalDatabase.OperationType.UPDATE, database.getName(), key, document)
      );
    }
  }
  ...
  public void handleDelete(Database database, String key) {
    ...
    if (!database.isSynced()) {
      CloudNet.getInstance().getClusterNodeServerProvider().sendPacket(
        new PacketServerLocalDatabase(PacketServerLocalDatabase.OperationType.DELETE, database.getName(), key, null)
      );
    }
  }
  ...
  public void handleClear(Database database) {
    ...
    if (!database.isSynced()) {
      CloudNet.getInstance().getClusterNodeServerProvider().sendPacket(
        new PacketServerLocalDatabase(PacketServerLocalDatabase.OperationType.CLEAR, database.getName(), null, null)
      );
    }
  }
}
