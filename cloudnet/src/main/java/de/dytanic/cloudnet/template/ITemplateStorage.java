...
/**
 * The template storage manage the management of service specific templates that
 * should copy or deploy on the implementation
 * <p>
 * The implementation allows to deploy, copy and convert to a zip compressed
 * array any template that should handle
 */
public interface ITemplateStorage extends AutoCloseable {

  /**
   * Deploys a zip compressed into a target template storage that should
   * decompressed and deploy on the target template
   *
   * @param zipInput the target zip compressed byte array within all files are
   * included for the target template
