...
public final class CommandCluster extends CommandDefault implements
  ITabCompleter {
  ...
  public void execute(ICommandSender sender, String command, String[] args,
    ...
    if (args.length == 0) {
      sender.sendMessage(
        " ",
        "ClusterId: " + getCloudNet().getConfig().getClusterConfig()
          .getClusterId(),
        " ",
        "cluster shutdown",
    ...
    if (args[0].toLowerCase().contains("push")) {
      if (args.length == 2) {
          ...
          for (ServiceTemplate serviceTemplate : storage.getTemplates()) {
            ...
            sender.sendMessage(
              LanguageManager.getMessage(
                "command-cluster-push-templates-from-local-success")
                .replace("%template%",
                  serviceTemplate.getStorage() + ":" + serviceTemplate
                    .getTemplatePath())
            );
          }
          return;
        ...
        if (args[1].equalsIgnoreCase("tasks")) {
          ...
          sender.sendMessage(
            LanguageManager.getMessage("command-cluster-push-tasks-success"));
          return;
        }
        ...
        if (args[1].equalsIgnoreCase("groups")) {
          ...
          sender.sendMessage(LanguageManager
            .getMessage("command-cluster-push-groups-success"));
          return;
        }
      }
      return;
    ...
    if (args[0].toLowerCase().contains("shutdown")) {
      for (IClusterNodeServer node : getCloudNet()
    ...
    if (args[0].toLowerCase().contains("nodes")) {
      for (IClusterNodeServer node : getCloudNet()
        .getClusterNodeServerProvider().getNodeServers()) {
        if (properties.containsKey("id") && !node.getNodeInfo().getUniqueId()
          .contains(properties.get("id"))) {
          continue;
        }
    ...
    if (args[0].toLowerCase().contains("add") && args.length == 4 && Validate
      .testStringParseToInt(args[3])) {
      NetworkCluster networkCluster = getCloudNet().getConfig()
        .getClusterConfig();
      ...
      getCloudNet().getClusterNodeServerProvider()
        .setClusterServers(networkCluster);

      sender.sendMessage(
        LanguageManager.getMessage("command-cluster-create-node-success"));
    }
  }
  ...
  private void displayNode(ICommandSender sender, IClusterNodeServer node) {
    ...
    if (node.getNodeInfoSnapshot() != null) {
      list.add(" ");
      list.add("* ClusterNodeInfoSnapshot from " + new SimpleDateFormat(
        "dd.MM.yyyy HH:mm:ss")
        .format(node.getNodeInfoSnapshot().getCreationTime()));

      list.addAll(Arrays.asList(
        "CloudServices (" + node.getNodeInfoSnapshot()
          .getCurrentServicesCount() + ") memory usage " +
          node.getNodeInfoSnapshot().getUsedMemory() + "/" + node
          .getNodeInfoSnapshot().getReservedMemory() + "/" + node
          .getNodeInfoSnapshot().getMaxMemory() + "MB",
        " ",
        "CPU usage: " + CPUUsageResolver.CPU_USAGE_OUTPUT_FORMAT.format(
          node.getNodeInfoSnapshot().getProcessSnapshot().getCpuUsage())
          + "%",
        "Threads: " + node.getNodeInfoSnapshot().getProcessSnapshot()
          .getThreads().size(),
        "Heap usage: " + (node.getNodeInfoSnapshot().getProcessSnapshot()
          .getHeapUsageMemory() / 1048576) + "/" +
          (node.getNodeInfoSnapshot().getProcessSnapshot()
            .getMaxHeapMemory() / 1048576) + "MB",
        "Loaded classes: " + node.getNodeInfoSnapshot().getProcessSnapshot()
          .getCurrentLoadedClassCount(),
        "Unloaded classes: " + node.getNodeInfoSnapshot().getProcessSnapshot()
          .getUnloadedClassCount(),
        "Total loaded classes: " + node.getNodeInfoSnapshot()
          .getProcessSnapshot().getTotalLoadedClassCount(),
        " ",
        "Extensions: ",
        Iterables.map(node.getNodeInfoSnapshot().getExtensions(),
          new Function<NetworkClusterNodeExtensionSnapshot, String>() {
            @Override
            public String apply(
              NetworkClusterNodeExtensionSnapshot networkClusterNodeExtensionSnapshot) {
      ...
      list.addAll(Arrays.asList(
        node.getNodeInfoSnapshot().getProperties().toPrettyJson()
          .split("\n")));
      list.add(" ");
    }
  ...
  public Collection<String> complete(String commandLine, String[] args,
    ...
    return args.length == 0 ? Arrays.asList(
      ...
      .map(getCloudNet().getCloudServiceManager().getCloudServices().keySet(),
