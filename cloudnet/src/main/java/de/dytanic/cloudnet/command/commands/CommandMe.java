  ...
  public void execute(ICommandSender sender, String command, String[] args,
    ...
    messages.addAll(Arrays.asList(
        ...
        "CloudNet " + CommandMe.class.getPackage().getImplementationTitle()
            + " " + CommandMe.class.getPackage().getImplementationVersion()
            + " by Dytanic",
        ...
        "CPU usage: (P/S) " + CPUUsageResolver.CPU_USAGE_OUTPUT_FORMAT
            .format(CPUUsageResolver.getProcessCPUUsage()) + "/" +
            CPUUsageResolver.CPU_USAGE_OUTPUT_FORMAT
                .format(CPUUsageResolver.getSystemCPUUsage()) + "/100%",
        "Node services memory allocation: " + getCloudNet()
            .getCurrentNetworkClusterNodeInfoSnapshot().getUsedMemory() + "/" +
        ...
        "Heap usage: " + (memoryMXBean.getHeapMemoryUsage().getUsed() / 1048576)
            + "/" + (memoryMXBean.getHeapMemoryUsage().getMax() / 1048576)
            + "MB",
        "Loaded classes: " + ManagementFactory.getClassLoadingMXBean()
            .getLoadedClassCount(),
        "Unloaded classes: " + ManagementFactory.getClassLoadingMXBean()
            .getUnloadedClassCount(),
        "Total loaded classes: " + ManagementFactory.getClassLoadingMXBean()
