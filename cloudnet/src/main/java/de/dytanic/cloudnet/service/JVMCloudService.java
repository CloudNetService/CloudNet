...
final class JVMCloudService implements ICloudService {
  ...
  private final List<ServiceRemoteInclusion> includes = Iterables
      .newArrayList();

  private final List<ServiceTemplate> templates = Iterables.newArrayList();

  private final List<ServiceDeployment> deployments = Iterables
      .newCopyOnWriteArrayList();

  private final Queue<ServiceRemoteInclusion> waitingIncludes = Iterables
      .newConcurrentLinkedQueue();

  private final Queue<ServiceTemplate> waitingTemplates = Iterables
      .newConcurrentLinkedQueue();

  private final DefaultServiceConsoleLogCache serviceConsoleLogCache = new DefaultServiceConsoleLogCache(
      this);
  ...
  JVMCloudService(ICloudServiceManager cloudServiceManager,
    ...
    this.waitingIncludes
        .addAll(Arrays.asList(this.serviceConfiguration.getIncludes()));
    this.waitingTemplates
        .addAll(Arrays.asList(this.serviceConfiguration.getTemplates()));

    this.deployments
        .addAll(Arrays.asList(this.serviceConfiguration.getDeployments()));

    this.groups.addAll(Arrays.asList(this.serviceConfiguration.getGroups()));
    this.configuredMaxHeapMemory = this.serviceConfiguration.getProcessConfig()
        .getMaxHeapMemorySize();

    this.serviceInfoSnapshot = this.lastServiceInfoSnapshot = createServiceInfoSnapshot(
        ServiceLifeCycle.DEFINED);

    this.directory =
        serviceConfiguration.isStaticService() ?
            new File(cloudServiceManager.getPersistenceServicesDirectory(),
                this.serviceId.getName())
            :
                new File(cloudServiceManager.getTempDirectory(),
                    this.serviceId.getName() + "#" + this.serviceId
  ...
  public void start() throws Exception {
    try {
      ...
      if (!CloudNet.getInstance().getConfig()
          .isParallelServiceStartSequence()) {
        try {
  ...
  public boolean isAlive() {
    return this.lifeCycle == ServiceLifeCycle.DEFINED
        ...
        (this.lifeCycle == ServiceLifeCycle.RUNNING && this.process != null
  ...
  private void initAndPrepareService() {
    if (this.lifeCycle == ServiceLifeCycle.DEFINED) {
      ...
      if (CloudNet.getInstance().getConfig().getServerSslConfig().isEnabled()) {
        try {
          ...
          if (ssl.getCertificatePath() != null) {
            ...
            if (file.exists()) {
              FileUtils
                  .copy(file, new File(this.directory, ".wrapper/certificate"));
            }
          }
          ...
          if (ssl.getPrivateKeyPath() != null) {
            ...
            if (file.exists()) {
              FileUtils
                  .copy(file, new File(this.directory, ".wrapper/privateKey"));
            }
          }
          ...
          if (ssl.getTrustCertificatePath() != null) {
            ...
            if (file.exists()) {
              FileUtils.copy(file,
                  new File(this.directory, ".wrapper/trustCertificate"));
            }
          }
        } catch (Exception ex) {
          ex.printStackTrace();
      ...
      this.lifeCycle = ServiceLifeCycle.PREPARED;
      CloudNetDriver.getInstance().getEventManager()
          .callEvent(new CloudServicePostPrepareEvent(this));

      serviceInfoSnapshot.setLifeCycle(ServiceLifeCycle.PREPARED);
      cloudServiceManager.getGlobalServiceInfoSnapshots()
          .put(serviceId.getUniqueId(), serviceInfoSnapshot);
      CloudNet.getInstance().sendAll(
      ...
      System.out.println(
          LanguageManager.getMessage("cloud-service-post-prepared-message")
              .replace("%task%", this.serviceId.getTaskName())
              .replace("%id%", this.serviceId.getUniqueId().toString()));
    }
  }
  ...
  private void start0() throws Exception {
    if (this.lifeCycle == ServiceLifeCycle.PREPARED
      ...
      System.out.println(
          LanguageManager.getMessage("cloud-service-pre-start-prepared-message")
              .replace("%task%", this.serviceId.getTaskName())
              .replace("%id%", this.serviceId.getUniqueId().toString()));
      CloudNetDriver.getInstance().getEventManager()
          .callEvent(new CloudServicePreStartPrepareEvent(this));
      ...
      this.serviceInfoSnapshot = this
          .createServiceInfoSnapshot(ServiceLifeCycle.PREPARED);
      this.cloudServiceManager.getGlobalServiceInfoSnapshots()
          .put(this.serviceInfoSnapshot.getServiceId().getUniqueId(),
              this.serviceInfoSnapshot);

      new JsonDocument()
          .append("connectionKey", this.connectionKey)
          .append("listener",
              CloudNet.getInstance().getConfig().getIdentity().getListeners()
          ...
          .append("sslConfig",
              CloudNet.getInstance().getConfig().getServerSslConfig())
          .write(new File(this.directory, ".wrapper/wrapper.json"));

      CloudNetDriver.getInstance().getEventManager()
          .callEvent(new CloudServicePostStartPrepareEvent(this));
      System.out.println(LanguageManager
          .getMessage("cloud-service-post-start-prepared-message")
          .replace("%task%", this.serviceId.getTaskName())
          .replace("%id%", this.serviceId.getUniqueId().toString()));

      System.out
          .println(LanguageManager.getMessage("cloud-service-pre-start-message")
              .replace("%task%", this.serviceId.getTaskName())
              .replace("%id%", this.serviceId.getUniqueId().toString()));
      CloudNetDriver.getInstance().getEventManager()
          .callEvent(new CloudServicePreStartEvent(this));
      ...
      CloudNetDriver.getInstance().getEventManager()
          .callEvent(new CloudServicePostStartEvent(this));
      System.out.println(
          LanguageManager.getMessage("cloud-service-post-start-message")
              .replace("%task%", this.serviceId.getTaskName())
              .replace("%id%", this.serviceId.getUniqueId().toString()));

      this.serviceInfoSnapshot.setLifeCycle(ServiceLifeCycle.RUNNING);
      CloudNet.getInstance().sendAll(
          new PacketClientServerServiceInfoPublisher(this.serviceInfoSnapshot,
              PacketClientServerServiceInfoPublisher.PublisherType.STARTED));
    }
  }

  private boolean hasAccessFromNode() {
    if (cloudServiceManager.getCurrentUsedHeapMemory() >= CloudNet.getInstance()
        .getConfig().getMaxMemory()) {
      if (CloudNet.getInstance().getConfig()
          .isRunBlockedServiceStartTryLaterAutomatic()) {
        ...
        System.out.println(LanguageManager
            .getMessage("cloud-service-manager-max-memory-error"));
      }

      return false;
    }

    if (CPUUsageResolver.getSystemCPUUsage() >= CloudNet.getInstance()
        .getConfig().getMaxCPUUsageToStartServices()) {
      if (CloudNet.getInstance().getConfig()
          .isRunBlockedServiceStartTryLaterAutomatic()) {
        CloudNet.getInstance().runTask(new Runnable() {
          @Override
        ...
        System.out.println(LanguageManager
            .getMessage("cloud-service-manager-cpu-usage-to-high-error"));
      }
  ...
  private ServiceInfoSnapshot createServiceInfoSnapshot(
      ServiceLifeCycle lifeCycle) {
    return new ServiceInfoSnapshot(
        System.currentTimeMillis(),
        this.serviceId,
        new HostAndPort(CloudNet.getInstance().getConfig().getHostAddress(),
            this.serviceConfiguration.getPort()),
        false,
        lifeCycle,
        new ProcessSnapshot(
  ...
  public void includeInclusions() {
    ...
    while (!this.waitingIncludes.isEmpty()) {
      ...
      if (inclusion != null && inclusion.getDestination() != null
          && inclusion.getUrl() != null) {
        try {
          System.out.println(LanguageManager
              .getMessage("cloud-service-include-inclusion-message")
              .replace("%task%", this.serviceId.getTaskName() + "")
              .replace("%id%", this.serviceId.getUniqueId().toString() + "")
          ...
          File cacheDestination = new File(
              ...
              Base64.getEncoder()
                  .encodeToString(EncryptTo.encryptToSHA256(inclusion.getUrl()))
          );
          ...
          try (InputStream inputStream = new FileInputStream(
            ...
            File destination = new File(this.directory,
                inclusion.getDestination());
            destination.getParentFile().mkdirs();

            try (OutputStream outputStream = Files
                .newOutputStream(destination.toPath())) {
  ...
  private boolean includeInclusions0(ServiceRemoteInclusion inclusion,
      File destination, byte[] buffer) throws Exception {
    URLConnection connection = new URL(inclusion.getUrl()).openConnection();

    CloudServicePreLoadInclusionEvent cloudServicePreLoadInclusionEvent = new CloudServicePreLoadInclusionEvent(
        this, inclusion, connection);
    CloudNetDriver.getInstance().getEventManager()
        .callEvent(cloudServicePreLoadInclusionEvent);
    ...
    if (inclusion.getProperties() != null) {
      if (inclusion.getProperties().contains("httpHeaders")) {
        JsonDocument document = inclusion.getProperties()
            .getDocument("httpHeaders");
    ...
    try (InputStream inputStream = connection
        .getInputStream(); OutputStream outputStream = Files
        .newOutputStream(destination.toPath())) {
      FileUtils.copy(inputStream, outputStream, buffer);
    }
  ...
  public void includeTemplates() {
    while (!this.waitingTemplates.isEmpty()) {
      ...
      if (template != null && template.getName() != null
          && template.getPrefix() != null && template.getStorage() != null) {
        ...
        CloudServiceTemplateLoadEvent cloudServiceTemplateLoadEvent = new CloudServiceTemplateLoadEvent(
            this, storage, template);
        CloudNetDriver.getInstance().getEventManager()
            .callEvent(cloudServiceTemplateLoadEvent);
        ...
        try {
          System.out.println(LanguageManager
              .getMessage("cloud-service-include-template-message")
              .replace("%task%", this.serviceId.getTaskName() + "")
              .replace("%id%", this.serviceId.getUniqueId().toString() + "")
  ...
  public void deployResources() {
    for (ServiceDeployment deployment : this.deployments) {
      if (deployment != null) {
        if (deployment.getTemplate() != null
            && deployment.getTemplate().getStorage() != null
            && deployment.getTemplate().getPrefix() != null &&
            deployment.getTemplate().getName() != null) {
          ITemplateStorage storage = getStorage(
              deployment.getTemplate().getStorage());

          CloudServiceDeploymentEvent cloudServiceDeploymentEvent = new CloudServiceDeploymentEvent(
              this, storage, deployment);
          CloudNetDriver.getInstance().getEventManager()
              .callEvent(cloudServiceDeploymentEvent);
          ...
          System.out.println(
              LanguageManager.getMessage("cloud-service-deploy-message")
                  .replace("%task%", this.serviceId.getTaskName() + "")
                  .replace("%id%", this.serviceId.getUniqueId().toString() + "")
                  .replace("%template%",
              ...
              Objects.requireNonNull(this.directory.listFiles(new FileFilter() {
                ...
                public boolean accept(File pathname) {
                  if (deployment.getExcludes() != null) {
                    return !deployment.getExcludes().contains(
                        pathname.isDirectory() ? pathname.getName() + "/"
                            : pathname.getName()) && !pathname
                        .getName().equals("wrapper.jar") && !pathname.getName()
                        .equals(".wrapper");
                  } else {
                    return true;
                  }
          ...
          if (storage instanceof LocalTemplateStorage) {
            CloudNet.getInstance()
                .deployTemplateInCluster(deployment.getTemplate(),
                    storage.toZipByteArray(deployment.getTemplate()));
          }
        }
      }
    }
  ...
  private void startApplication() throws Exception {
    ...
    commandArguments.addAll(Arrays.asList(
        ...
        "-Dcloudnet.wrapper.receivedMessages.language=" + LanguageManager
            .getLanguage()
    ));

    File wrapperFile = new File(System.getProperty("cloudnet.tempDir", "temp"),
        "caches/wrapper.jar");

    commandArguments
        .addAll(this.serviceConfiguration.getProcessConfig().getJvmOptions());
    commandArguments.addAll(Arrays.asList(
        "-Xmx" + this.serviceConfiguration.getProcessConfig()
            .getMaxHeapMemorySize() + "M",
        "-javaagent:" + wrapperFile.getAbsolutePath(),
        "-cp",
        "\"" + System.getProperty("cloudnet.launcher.driver.dependencies")
  ...
  private void postConfigureServiceEnvironmentStartParameters(
      List<String> commandArguments) throws Exception {
    ...
    switch (this.serviceConfiguration.getProcessConfig().getEnvironment()) {
        ...
        this.rewriteServiceConfigurationFile(file, new UnaryOperator<String>() {
          ...
          public String apply(String s) {
            if (s.startsWith("  host: ")) {
              s = "  host: " + CloudNet.getInstance().getConfig()
                  .getHostAddress() + ":" + serviceConfiguration.getPort();
            }
        ...
        this.rewriteServiceConfigurationFile(file, new UnaryOperator<String>() {
          ...
          public String apply(String s) {
            if (value.getValue() && s.startsWith("bind =")) {
              ...
              return "bind = \"" + CloudNet.getInstance().getConfig()
                  .getHostAddress() + ":" + serviceConfiguration.getPort()
                  + "\"";
            }
      ...
      case MINECRAFT_SERVER: {
        ...
        properties.setProperty("server-ip",
            CloudNet.getInstance().getConfig().getHostAddress());
        ...
        try (OutputStream outputStream = new FileOutputStream(file);
          ...
          properties
              .store(outputStreamWriter, "Auto Eula agreement by CloudNet");
        }
      }
      break;
      case NUKKIT: {
      ...
      case GO_MINT: {
        ...
        this.rewriteServiceConfigurationFile(file, new UnaryOperator<String>() {
          ...
          public String apply(String s) {
            if (s.startsWith("  ip: ")) {
              s = "  ip: " + CloudNet.getInstance().getConfig()
                  .getHostAddress();
            }
  ...
  private void copyDefaultFile(String from, File target) throws Exception {
    if (!target.exists()) {
      ...
      try (InputStream inputStream = JVMCloudService.class.getClassLoader()
  ...
  private void rewriteServiceConfigurationFile(File file,
      UnaryOperator<String> unaryOperator) throws Exception {
  ...
  private int stop0(boolean force) {
    if (this.lifeCycle == ServiceLifeCycle.RUNNING) {
      System.out
          .println(LanguageManager.getMessage("cloud-service-pre-stop-message")
              .replace("%task%", this.serviceId.getTaskName())
              .replace("%id%", this.serviceId.getUniqueId().toString()));
      CloudNetDriver.getInstance().getEventManager()
          .callEvent(new CloudServicePreStopEvent(this));
      ...
      this.lifeCycle = ServiceLifeCycle.STOPPED;
      CloudNetDriver.getInstance().getEventManager()
          .callEvent(new CloudServicePostStopEvent(this, exitValue));
      System.out
          .println(LanguageManager.getMessage("cloud-service-post-stop-message")
              .replace("%task%", this.serviceId.getTaskName())
              .replace("%id%", this.serviceId.getUniqueId().toString())
              .replace("%exit_value%", exitValue + ""));

      this.serviceInfoSnapshot = createServiceInfoSnapshot(
  ...
  private void delete0() {
    ...
    System.out
        .println(LanguageManager.getMessage("cloud-service-pre-delete-message")
            .replace("%task%", this.serviceId.getTaskName())
            .replace("%id%", this.serviceId.getUniqueId().toString()));
    CloudNetDriver.getInstance().getEventManager()
        .callEvent(new CloudServicePreDeleteEvent(this));
    ...
    this.cloudServiceManager.getCloudServices()
        .remove(this.serviceId.getUniqueId());
    this.cloudServiceManager.getGlobalServiceInfoSnapshots()
        .remove(this.serviceId.getUniqueId());

    CloudNetDriver.getInstance().getEventManager()
        .callEvent(new CloudServicePostDeleteEvent(this));
    System.out
        .println(LanguageManager.getMessage("cloud-service-post-delete-message")
            .replace("%task%", this.serviceId.getTaskName())
            .replace("%id%", this.serviceId.getUniqueId().toString()));
  ...
  private ITemplateStorage getStorage(String storageName) {
    ...
    if (CloudNetDriver.getInstance().getServicesRegistry()
        .containsService(ITemplateStorage.class, storageName)) {
      storage = CloudNetDriver.getInstance().getServicesRegistry()
          .getService(ITemplateStorage.class, storageName);
    } else {
      storage = CloudNetDriver.getInstance().getServicesRegistry()
