...
public final class DefaultClusterNodeServer implements IClusterNodeServer {
  ...
  DefaultClusterNodeServer(DefaultClusterNodeServerProvider provider,
  ...
  @Override
  public void sendClusterChannelMessage(String channel, String message,
      JsonDocument header, byte[] body) {
    saveSendPacket(
        new PacketServerClusterChannelMessage(channel, message, header, body));
  }

  @Override
  public void sendCustomChannelMessage(String channel, String message,
      JsonDocument data) {
  ...
  @Override
  public boolean isAcceptableConnection(INetworkChannel channel,
      String nodeId) {
    return channel != null && this.channel == null && this.nodeInfo
        .getUniqueId().equals(nodeId);
  }
    ...
    if (this.channel != null && this.isConnected()) {
      try {
        return CloudNetDriver.getInstance()
            .sendCallablePacketWithAsDriverSyncAPI(channel,
  ...
  public ServiceInfoSnapshot createCloudService(ServiceTask serviceTask) {
    ...
    if (this.channel != null && this.isConnected()) {
      try {
        return CloudNetDriver.getInstance()
            .sendCallablePacketWithAsDriverSyncAPI(this.channel,
                new JsonDocument(PacketConstants.SYNC_PACKET_ID_PROPERTY,
                    "create_CloudService_by_serviceTask")
                    .append("serviceTask", serviceTask), new byte[0],
        ...
        return CloudNetDriver.getInstance()
            .sendCallablePacketWithAsDriverSyncAPI(this.channel,
                ...
                new Function<Pair<JsonDocument, byte[]>, Queue<String>>() {
                  @Override
                  public Queue<String> apply(
                      Pair<JsonDocument, byte[]> documentPair) {
                    return documentPair.getFirst().get("cachedLogMessages",
    ...
    if (this.channel != null && this.isConnected()) {
      try {
        return CloudNetDriver.getInstance().sendCallablePacket(this.channel,
            ...
            new JsonDocument()
                .append(PacketConstants.SYNC_PACKET_ID_PROPERTY,
                    "get_reserved_task_ids")
                .append("task", task),
            new byte[0],
            new Function<Pair<JsonDocument, byte[]>, Collection<Integer>>() {
              ...
              public Collection<Integer> apply(
                  Pair<JsonDocument, byte[]> documentPair) {
                return documentPair.getFirst()
                    .get("taskIds", TYPE_COLLECTION_INTEGER);
              }
            }
        ).get(5, TimeUnit.SECONDS);
      } catch (InterruptedException | ExecutionException | TimeoutException e) {
