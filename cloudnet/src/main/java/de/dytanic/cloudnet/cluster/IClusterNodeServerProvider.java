 ...
 */
@Deprecated
public interface IClusterNodeServerProvider extends AutoCloseable, PacketSender {

    /**
     * Returns the represent nodes that are configured on the application.
     * The nodes shouldn't be online
     */
    Collection<ClusterNodeServer> getNodeServers();
     ...
     */
    ClusterNodeServer getNodeServer(String uniqueId);
     ...
     * @return the IClusterNodeServer instance or null if the node doesn't registered
     */
    ClusterNodeServer getNodeServer(INetworkChannel channel);

    default ClusterNodeServer getNodeServer(NetworkChannel channel) {
        return getNodeServer((INetworkChannel) channel);
    }
