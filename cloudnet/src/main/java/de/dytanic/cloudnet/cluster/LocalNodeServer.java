...
public class LocalNodeServer extends DefaultNodeServer implements NodeServer {
    ...
    @Override
    public @Nullable SpecificCloudServiceProvider getCloudServiceProvider(@NotNull ServiceInfoSnapshot serviceInfoSnapshot) {
        ICloudService service = this.cloudNet.getCloudServiceManager().getCloudService(serviceInfoSnapshot.getServiceId().getUniqueId());
        return service == null
