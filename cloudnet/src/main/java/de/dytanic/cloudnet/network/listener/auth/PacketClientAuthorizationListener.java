                    ...
                    if (credentials.contains("clusterId") && credentials.contains("clusterNode") &&
                        ...
                        for (IClusterNodeServer clusterNodeServer : this.getCloudNet().getClusterNodeServerProvider().getNodeServers()) {
                            if (clusterNodeServer.isAcceptableConnection(channel, clusterNode.getUniqueId())) {
                                ...
                                channel.getPacketRegistry().addListener(PacketConstants.CLUSTER_NODE_START_SERVICE_CHANNEL, new PacketServerStartServiceFromConfigurationListener(CloudNet.getInstance()));
                ...
                case WRAPPER_TO_NODE:
                    if (credentials.contains("connectionKey") && credentials.contains("serviceId")) {
                        ...
                        if (connectionKey != null && cloudService != null && cloudService.getConnectionKey().equals(connectionKey) &&
                            ...
                            //- packet channel registry
                            channel.getPacketRegistry().addListener(PacketConstants.CHANNEL_MESSAGING_CHANNEL, new PacketServerChannelMessageListener(true));
                            ...
                            channel.getPacketRegistry().addListener(PacketConstants.INTERNAL_DATABASE_API_CHANNEL, new PacketServerRemoteDatabaseActionListener());
