...
/**
 * The ProgressBar represents a progressing situation of a program part. For
 * example
 * <p>
 * CAProgressBar progressBar = new CAProgressBar( '█', "[%percent%%] ", " |
 * %value%/%target%MB Downloaded... (%time%)", 3, 2048, false );
 * <p>
 * TaskScheduler.runtimeScheduler().schedule(new Runnable() {
 *
 * @Override public void run() { if (progressBar.getProgressValue() <
 * progressBar.getTargetGoal()) progressBar.setProgressValue(progressBar.getProgressValue()
 * + 1); } }, 5, -1); consoleProvider.invokeConsoleAnimation(progressBar);
 */
@Data
@ToString
