...
public class CommandSigns implements CommandExecutor {
    ...
    public @NotNull CommandResult execute(@NotNull CommandSource src, @NotNull CommandContext args) {
        ...
        if (type != null) {
            if (type.equalsIgnoreCase("create") && targetGroup != null) {
                ...
                Sign sign = this.signManagement.getSignAt((org.spongepowered.api.block.tileentity.Sign) hit.get().getLocation().getTileEntity().get());
                if (sign != null) {
                    ...
                    Sign createdSign = this.signManagement.createSign((org.spongepowered.api.block.tileentity.Sign) hit.get().getLocation().getTileEntity().get(),
    ...
    protected @NotNull Optional<BlockRayHit<World>> getTargetBlock(@NotNull Player player) {
        Optional<BlockRayHit<World>> hit = BlockRay.from(player)
                ...
                .whilst(lastHit -> {
                    BlockType blockType = lastHit.getExtent().getBlockType(lastHit.getBlockX(), lastHit.getBlockY(), lastHit.getBlockZ());
                    return blockType.equals(BlockTypes.STANDING_SIGN) || blockType.equals(BlockTypes.WALL_SIGN);
                })
                .end();
        if (!hit.isPresent() || !(hit.get().getLocation().getTileEntity().orElse(null) instanceof org.spongepowered.api.block.tileentity.Sign)) {
