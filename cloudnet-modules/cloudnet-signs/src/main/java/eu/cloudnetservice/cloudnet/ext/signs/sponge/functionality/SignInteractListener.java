...
public class SignInteractListener {
    ...
    public SignInteractListener(Object plugin, ServiceSignManagement<org.spongepowered.api.block.tileentity.Sign> signManagement) {
        this.plugin = plugin;
    ...
    public void handle(InteractBlockEvent event, @First Player player) {
        ...
        if (blockType.equals(BlockTypes.WALL_SIGN) || blockType.equals(BlockTypes.STANDING_SIGN)) {
            if (event.getTargetBlock().getLocation().isPresent()
                    ...
                    && event.getTargetBlock().getLocation().get().getTileEntity().orElse(null) instanceof org.spongepowered.api.block.tileentity.Sign) {
                Sign sign = this.signManagement.getSignAt((org.spongepowered.api.block.tileentity.Sign) event.getTargetBlock().getLocation().get().getTileEntity().get());
                if (sign != null) {
                    boolean canConnect = this.signManagement.canConnect(sign, player::hasPermission);

                    SpongeCloudSignInteractEvent interactEvent = new SpongeCloudSignInteractEvent(Cause.of(EventContext.empty(), plugin),
                    ...
                    if (!interactEvent.isCancelled() && interactEvent.getTarget().isPresent()) {
                        this.signManagement.getSignsConfiguration().sendMessage("server-connecting-message",
                                m -> player.sendMessage(Text.of(m)), m -> m.replace("%server%", interactEvent.getTarget().get().getName()));
