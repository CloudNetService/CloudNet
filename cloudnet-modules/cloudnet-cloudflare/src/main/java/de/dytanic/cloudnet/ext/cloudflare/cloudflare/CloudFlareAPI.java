...
public class CloudFlareAPI implements AutoCloseable {
    ...
    @Nullable
    public DnsRecordDetail createRecord(@NotNull UUID serviceUniqueId, @NotNull CloudflareConfigurationEntry configuration, @NotNull DNSRecord record) {
        Preconditions.checkNotNull(serviceUniqueId, "serviceUniqueId");
        Preconditions.checkNotNull(configuration, "configuration");
        Preconditions.checkNotNull(record, "record");

        try {
            HttpURLConnection connection = this.prepareConnection(String.format(ZONE_RECORDS_ENDPOINT, configuration.getZoneId()), "POST", configuration);
            ...
            if (result.getBoolean("success") && content != null) {
                ...
                if (id != null) {
                    CloudNet.getInstance().getLogger().debug("Successfully created record with id " + id + " based on " + record + " (configuration: " + configuration + ")");
    ...
    public boolean deleteRecord(@NotNull CloudflareConfigurationEntry configuration, @NotNull String id) {
        ...
        try {
            HttpURLConnection connection = this.prepareConnection(String.format(ZONE_RECORDS_MANAGEMENT_ENDPOINT, configuration.getZoneId(), id), "DELETE", configuration);
            JsonDocument result = this.sendRequestAndReadResponse(connection);

            JsonDocument content = result.getDocument("result");
            if (content != null && content.contains("id")) {
                CloudNet.getInstance().getLogger().debug("Successfully deleted record " + id + " for configuration " + configuration);
                return true;
            }

            CloudNet.getInstance().getLogger().debug("Unable to delete record " + id + ", response: " + result);
        } catch (IOException exception) {
            CloudNet.getInstance().getLogger().fatal("Error while deleting dns record for configuration " + configuration, exception);
        }
    ...
    @NotNull
    protected HttpURLConnection prepareConnection(@NotNull String endpoint, @NotNull String method, @NotNull CloudflareConfigurationEntry entry) throws IOException {
    ...
    @NotNull
    protected JsonDocument sendRequestAndReadResponse(@NotNull HttpURLConnection connection, @NotNull DNSRecord record) throws IOException {
        Preconditions.checkNotNull(connection, "connection");
        Preconditions.checkNotNull(record, "record");
    ...
    @NotNull
    protected JsonDocument sendRequestAndReadResponse(@NotNull HttpURLConnection connection, @Nullable String data) throws IOException {
        Preconditions.checkNotNull(connection, "connection");
