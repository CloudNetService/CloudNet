...
public final class BungeeProxyLoginConfigurationImplListener implements
  Listener {

  @EventHandler
  public void handle(ProxyPingEvent event) {
    SyncProxyProxyLoginConfiguration syncProxyProxyLoginConfiguration = BungeeCloudNetSyncProxyPlugin
    ...
    if (syncProxyProxyLoginConfiguration != null) {
      ...
      if (syncProxyProxyLoginConfiguration.isMaintenance()) {
        if (syncProxyProxyLoginConfiguration.getMaintenanceMotds() != null
          && !syncProxyProxyLoginConfiguration.getMaintenanceMotds()
          .isEmpty()) {
          syncProxyMotd = syncProxyProxyLoginConfiguration.getMaintenanceMotds()
            .get(random.nextInt(
              syncProxyProxyLoginConfiguration.getMaintenanceMotds()
                .size()));
        }
      } else {
        if (syncProxyProxyLoginConfiguration.getMotds() != null
          && !syncProxyProxyLoginConfiguration.getMotds().isEmpty()) {
          syncProxyMotd = syncProxyProxyLoginConfiguration.getMotds()
            .get(random.nextInt(
      ...
      if (syncProxyMotd != null) {
        int onlinePlayers = BungeeCloudNetSyncProxyPlugin.getInstance()
          .getSyncProxyOnlineCount();

        ServerPing serverPing = createServerPingInstance(
          event,
          ChatColor.translateAlternateColorCodes('&',
            syncProxyMotd.getFirstLine() + "\n" + syncProxyMotd
              .getSecondLine() + "")
            .replace("%proxy%",
            ...
            .replace("%node%",
              Wrapper.getInstance().getServiceId().getNodeUniqueId()
                + ""),
  ...
  public void handle(LoginEvent event) {
    ...
    if (syncProxyProxyLoginConfiguration != null) {
      LoginPendingConnectionCommandSender loginEventCommandSender = new LoginPendingConnectionCommandSender(
        event, getUniqueIdOfPendingConnection(event.getConnection()));

      if (syncProxyProxyLoginConfiguration.isMaintenance()
        && syncProxyProxyLoginConfiguration.getWhitelist() != null) {
        if (syncProxyProxyLoginConfiguration.getWhitelist()
          .contains(event.getConnection().getName())) {
          return;
        }

        UUID uniqueId = getUniqueIdOfPendingConnection(event.getConnection());

        if ((uniqueId != null && syncProxyProxyLoginConfiguration.getWhitelist()
          .contains(uniqueId.toString())) ||
          loginEventCommandSender
            .hasPermission("cloudnet.syncproxy.maintenance")) {
          return;
        }
      ...
      if (BungeeCloudNetSyncProxyPlugin.getInstance().getSyncProxyOnlineCount()
        >= syncProxyProxyLoginConfiguration.getMaxPlayers() &&
        !loginEventCommandSender
          .hasPermission("cloudnet.syncproxy.fullljoin")) {
        event.setCancelled(true);
        event.setCancelReason(ChatColor.translateAlternateColorCodes('&',
          SyncProxyConfigurationProvider.load().getMessages()
  ...
  private ServerPing createServerPingInstance(ProxyPingEvent event,
    String motd,
    String protocolText, int onlinePlayers, int maxPlayers,
    String[] playerInfo) {
    ...
    //up to 1.7
    try {
      Class<?> protocolClass = Class
        .forName("net.md_5.bungee.api.ServerPing$Protocol");
      Class<?> playersClass = Class
        .forName("net.md_5.bungee.api.ServerPing$Players");
      Class<?> playerInfoClass = Class
        .forName("net.md_5.bungee.api.ServerPing$PlayerInfo");
      ...
      Constructor<?> playerInfoClassConstructor = playerInfoClass
        .getConstructor(String.class, String.class);

      Object array = Array.newInstance(playerInfoClass,
        playerInfo != null ? playerInfo.length : 0);

      if (playerInfo != null) {
        for (int i = 0; i < playerInfo.length; i++) {
          Array.set(array, i, playerInfoClassConstructor.newInstance(
            ChatColor.translateAlternateColorCodes('&', playerInfo[i]),
      ...
      Method methodGetFaviconObject = ServerPing.class
        .getDeclaredMethod("getFaviconObject");
      methodGetFaviconObject.setAccessible(true);

      Method methodGetVersion = ServerPing.class
      ...
      return ServerPing.class
        .getConstructor(protocolClass, playersClass, String.class,
          faviconClass)
        .newInstance(
          protocolText == null && protocol != null ? protocol
            : protocolClass.getConstructor(String.class, int.class)
              .newInstance(
                ChatColor.translateAlternateColorCodes('&',
                  (protocolText == null ?
                    ProxyServer.getInstance().getName() + " "
                      + ProxyServer.getInstance()
                      .getGameVersion() : protocolText)
                    .replace("%proxy%",
                    ...
                    + ""),
                (protocolText == null ? ProxyServer.getInstance()
                  .getProtocolVersion() : 1)),
          //supports all BungeeCord versions
          playersClass
            .getConstructor(int.class, int.class, array.getClass())
            .newInstance(maxPlayers, onlinePlayers, array),
    ...
    try {
      ...
      Constructor<ServerPing> constructor = ServerPing.class
        .getConstructor(byte.class, String.class, String.class, int.class,
          int.class);

      return constructor.newInstance(method.invoke(event.getResponse()),
        ProxyServer.getInstance().getGameVersion(), motd, onlinePlayers,
        maxPlayers);
    } catch (Exception ignored) {
    }
  ...
  private UUID getUniqueIdOfPendingConnection(
