  ...
  public void handle(ProxyPingEvent event) {
    ...
    if (syncProxyProxyLoginConfiguration != null) {
      ...
      if (syncProxyMotd != null) {
        int onlinePlayers = VelocityCloudNetSyncProxyPlugin.getInstance()
        ...
        event.setPing(new ServerPing(
          ...
          new ServerPing.Players(
            ...
            syncProxyMotd.isAutoSlot() ?
              (syncProxyMotd.getAutoSlotMaxPlayersDistance()
                + onlinePlayers) :
              syncProxyProxyLoginConfiguration.getMaxPlayers(),
            ...
            .of((syncProxyMotd.getFirstLine() + "\n" + syncProxyMotd
              ...
              .replace("%node%",
                Wrapper.getInstance().getServiceId().getNodeUniqueId()
                  + "")
              .replace("&", "§")),
          event.getPing().getFavicon().isPresent() ? event.getPing()
            .getFavicon().get() : null,
          event.getPing().getModinfo().isPresent() ? event.getPing()
            .getModinfo().get() : null
        ));
      }
    }
    ...
    if (syncProxyProxyLoginConfiguration != null) {
      if (syncProxyProxyLoginConfiguration.isMaintenance()
        ...
        if (syncProxyProxyLoginConfiguration.getWhitelist()
          .contains(event.getPlayer().getUsername()) ||
        ...
        event.setResult(LoginEvent.ComponentResult.denied(TextComponent
          .of((SyncProxyConfigurationProvider.load().getMessages()
            .get("player-login-not-whitelisted") + "").replace("&", "§"))));
        return;
      }

      if (VelocityCloudNetSyncProxyPlugin.getInstance()
        .getSyncProxyOnlineCount() >= syncProxyProxyLoginConfiguration
        .getMaxPlayers() &&
