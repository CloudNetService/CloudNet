  ...
  public void execute(@MaybePresent CommandSource source,
    ...
    if (VelocityCloudNetHelper.isOnAFallbackInstance(player)) {
      source.sendMessage(TextComponent
          .of(BridgeConfigurationProvider.load().getMessages()
              .get("command-hub-already-in-hub").replace("&", "§")));
      return;
    }

    String server = VelocityCloudNetHelper.filterServiceForPlayer(player,
        player.getCurrentServer().get().getServerInfo().getName());

    if (server != null) {
      RegisteredServer registeredServer = VelocityCloudNetHelper
          .getProxyServer().getServer(server).get();

      if (registeredServer == null) {
        source.sendMessage(TextComponent
            .of(BridgeConfigurationProvider.load().getMessages()
                .get("command-hub-no-server-found").replace("&", "§")));
        return;
      }
