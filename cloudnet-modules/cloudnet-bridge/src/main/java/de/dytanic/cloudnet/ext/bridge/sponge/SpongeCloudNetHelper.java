...
public final class SpongeCloudNetHelper {
  ...
  public static void initProperties(ServiceInfoSnapshot serviceInfoSnapshot) {
    ...
    serviceInfoSnapshot.getProperties()
        ...
        .append("Extra", extra)
        .append("State", state)
        .append("Outgoing-Channels", Sponge.getChannelRegistrar()
            .getRegisteredChannels(Platform.Type.SERVER))
        .append("Incoming-Channels", Sponge.getChannelRegistrar()
            .getRegisteredChannels(Platform.Type.CLIENT))
        .append("Online-Mode", Sponge.getServer().getOnlineMode())
        .append("Whitelist-Enabled", Sponge.getServer().hasWhitelist())
        .append("Players", Iterables.map(Sponge.getServer().getOnlinePlayers(),
            new Function<Player, SpongeCloudNetPlayerInfo>() {
              @Override
              public SpongeCloudNetPlayerInfo apply(Player player) {
                Location<World> location = player.getLocation();

                Optional<ExperienceHolderData> holderData = player
                    .get(ExperienceHolderData.class);
        ...
        .append("Plugins", Iterables
            .map(Sponge.getGame().getPluginManager().getPlugins(),
                new Function<PluginContainer, PluginInfo>() {
                  ...
                  public PluginInfo apply(PluginContainer pluginContainer) {
                    PluginInfo pluginInfo = new PluginInfo(
                        ...
                        pluginContainer.getVersion().isPresent()
                            ? pluginContainer.getVersion().get() : null);

                    pluginInfo.getProperties()
                        .append("name", pluginContainer.getName())
                        .append("authors", pluginContainer.getAuthors())
                        .append("url", pluginContainer.getUrl().isPresent()
                            ? pluginContainer.getUrl().get() : null)
                        .append("description",
                            pluginContainer.getDescription().isPresent()
                                ? pluginContainer.getDescription().get()
                                : null);

                    return pluginInfo;
                  }
                }))
        .append("Worlds", Iterables.map(Sponge.getServer().getWorlds(),
            new Function<World, WorldInfo>() {
              @Override
              public WorldInfo apply(World world) {
                return new WorldInfo(world.getUniqueId(), world.getName(),
                    world.getDifficulty().getName(), world.getGameRules());
              }
            }));
  }
