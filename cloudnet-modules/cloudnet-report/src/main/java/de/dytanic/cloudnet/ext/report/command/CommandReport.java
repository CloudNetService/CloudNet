...
public final class CommandReport extends Command {
  ...
  private final Gson gson = new GsonBuilder()
    .registerTypeAdapterFactory(TypeAdapters
      .newFactory(JsonDocument.class, new JsonDocumentTypeAdapter()))
    .setPrettyPrinting()
    .serializeNulls()
    .create();
  ...
  public CommandReport() {
    ...
    this.description = LanguageManager
      .getMessage("module-report-command-report-description");
  }
  ...
  public void execute(ICommandSender sender, String command, String[] args,
    ...
    File directory = new File(
      CloudNetReportModule.getInstance().getModuleWrapper().getDataFolder(),
      "reports");
    directory.mkdirs();
    ...
    try (FileWriter fileWriter = new FileWriter(file, false);
      ...
      sender.sendMessage(LanguageManager
        .getMessage("module-report-command-report-post-success")
  ...
  private void postOutput(Writer w, long millis) throws IOException {
    try (PrintWriter writer = new PrintWriter(w, true)) {
      writer.println(
        "Report from " + new SimpleDateFormat("dd.MM.yyyy HH:mm:ss")
          .format(millis));
      writer.println();
      writer.println(
        "Last Event Publisher Class: " + CloudNetReportModule.getInstance()
          .getEventClass());
      writer.println();

      writer.println("Last log lines");
      for (LogEntry logEntry : CloudNet.getInstance()
        .getQueuedConsoleLogHandler().getCachedQueuedLogEntries()) {
      ...
      Collection<Map.Entry<Thread, StackTraceElement[]>> threads = Thread
        .getAllStackTraces().entrySet();

      writer.println("Threads: " + threads.size());
      for (Map.Entry<Thread, StackTraceElement[]> entry : threads) {
        writer.println(
          "Thread " + entry.getKey().getId() + " | " + entry.getKey()
            .getName() + " | " + entry.getKey().getState());
        writer.println(
          "- Daemon: " + entry.getKey().isDaemon() + " | isAlive: " + entry
            .getKey().isAlive() + " | Priority: " + entry.getKey()
            .getPriority());
        writer.println("- Context ClassLoader: " + (
          entry.getKey().getContextClassLoader() != null ?
            entry.getKey().getContextClassLoader().getClass().getName()
            : "not defined"));
        writer.println(
          "- ThreadGroup: " + entry.getKey().getThreadGroup().getName());
        writer.println();
      ...
      for (IClusterNodeServer clusterNodeServer : CloudNet.getInstance()
        ...
        writer.println("Node: " + clusterNodeServer.getNodeInfo().getUniqueId()
          + " | Connected: " + clusterNodeServer.isConnected());
      ...
      writer.println(
        "###################################################################################");
      writer.println(
        "Services: " + CloudNetDriver.getInstance().getCloudServices()
          .size());
      for (ServiceInfoSnapshot serviceInfoSnapshot : CloudNetDriver
        .getInstance().getCloudServices()) {
        writer.println(
          "* Service " + serviceInfoSnapshot.getServiceId().getName() + " | "
        ...
        for (String entry : CloudNetDriver.getInstance()
      ...
      for (CommandInfo commandInfo : CloudNet.getInstance().getCommandMap()
        .getCommandInfos()) {
      ...
      for (IModuleWrapper moduleWrapper : CloudNetDriver.getInstance()
        .getModuleProvider().getModules()) {
        writer.println(moduleWrapper.getModuleConfiguration().getName() + " | "
          + moduleWrapper.getModuleLifeCycle());
        writer.println();
        ...
        for (Map.Entry<ModuleLifeCycle, List<IModuleTaskEntry>> moduleLifeCycleListEntry : moduleWrapper
          ...
          for (IModuleTaskEntry moduleTaskEntry : moduleLifeCycleListEntry
            .getValue()) {
            writer.println(
              "Order: " + moduleTaskEntry.getTaskInfo().order() + " | "
                + moduleTaskEntry.getHandler().getName());
          }
        }
      ...
      for (Class<?> c : CloudNetDriver.getInstance().getServicesRegistry()
        .getProvidedServices()) {
        writer.println("Registry Item Class: " + c.getName());

        for (Object item : CloudNetDriver.getInstance().getServicesRegistry()
          .getServices(c)) {
