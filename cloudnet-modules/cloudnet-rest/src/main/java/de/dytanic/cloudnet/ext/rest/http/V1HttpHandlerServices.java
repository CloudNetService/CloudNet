    ...
    if (context.request().pathParameters().containsKey("uuid")) {
            ...
            new Predicate<ServiceInfoSnapshot>() {
              ...
              public boolean test(ServiceInfoSnapshot serviceInfoSnapshot) {
                return serviceInfoSnapshot.getServiceId().getName()
                  .contains(
                    context.request().pathParameters().get("uuid"));
              }
            });
      }
      ...
      if (context.request().pathParameters().containsKey("operation")) {
        switch (context.request().pathParameters().get("operation")
          ...
          break;
          case "log": {
            Queue<String> queue = CloudNetDriver.getInstance()
            ...
            public boolean test(ServiceInfoSnapshot serviceInfoSnapshot) {
              ...
              if (context.request().queryParameters().containsKey("task")
                ...
                !context.request().queryParameters().get("task")
                  .contains(serviceInfoSnapshot.getServiceId()
                    .getTaskName())) {
                return false;
              }
