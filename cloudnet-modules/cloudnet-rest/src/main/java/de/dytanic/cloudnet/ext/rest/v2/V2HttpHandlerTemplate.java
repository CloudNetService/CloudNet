...
public class V2HttpHandlerTemplate extends V2HttpHandler {
        ...
        this.handleWithFileTemplateContext(context, (template, storage, path) -> {
            ...
            if (stream == null) {
                ...
                this.ok(context, "application/octet-stream")
                        .header("Content-Disposition", String.format("attachment%s", fileName == null ? "" : "; filename=" + fileName))
                        .body(stream)
                        .context()
                        .closeAfter(true)
                        .cancelNext();
    ...
    protected void handleInstallationRequest(IHttpContext context) {
        this.handleWithTemplateContext(context, (template, storage) -> {
            ...
            if (versionType == null) {
                versionType = this.getCloudNet().getServiceVersionProvider().getServiceVersionType(body.getString("typeName", "")).orElse(null);
                if (versionType == null) {
                    this.badRequest(context)
            ...
            if (this.getCloudNet().getServiceVersionProvider().installServiceVersion(versionType, version, template, forceInstall)) {
    ...
    protected void handleWithTemplateContext(IHttpContext context, ThrowableBiConsumer<ServiceTemplate, SpecificTemplateStorage, Exception> handler) {
    ...
    protected void handleWithFileTemplateContext(IHttpContext context, ThrowableTriConsumer<ServiceTemplate, SpecificTemplateStorage, String, Exception> handler) {
        this.handleWithTemplateContext(context, (template, storage) -> {
