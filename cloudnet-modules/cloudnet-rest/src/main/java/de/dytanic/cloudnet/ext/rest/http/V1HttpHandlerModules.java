  ...
  public void handleGet(String path, IHttpContext context) throws Exception {
    if (context.request().pathParameters().containsKey("name")) {
      context
        ...
        .body(GSON.toJson(Iterables.filter(
          CloudNetDriver.getInstance().getModuleProvider().getModules(),
          new Predicate<IModuleWrapper>() {
            @Override
            public boolean test(IModuleWrapper moduleWrapper) {
              return context.request().pathParameters().get("name")
                .contains(
      ...
      context
        ...
        .header("Content-Type", "application/json")
        .statusCode(200)
        .body(GSON.toJson(Iterables.filter(Iterables.map(
          CloudNetDriver.getInstance().getModuleProvider().getModules(),
          new Function<IModuleWrapper, ModuleConfiguration>() {
            @Override
          ...
          public boolean test(ModuleConfiguration moduleConfiguration) {
            if (context.request().queryParameters().containsKey("group") &&
              !containsStringElementInCollection(
                context.request().queryParameters().get("group"),
            ...
            if (context.request().queryParameters().containsKey("version") &&
              !containsStringElementInCollection(
                context.request().queryParameters().get("version"),
