...
/**
 * The LanguageManager is a static service, which handles messages from
 * different languages, which are registered or loaded there
 */
public final class LanguageManager {
  ...
  /**
   * The current language, which the getMessage() method will the message
   * return
   */
  @Getter
  @Setter
  ...
  /**
   * Resolve and returns the following message in the language which is
   * currently set as member "language"
   *
   * @param property the following message property, which should sort out
   * @return the message which is defined in language cache or a fallback
   * message like "MESSAGE OR LANGUAGE NOT FOUND!"
   */
  ...
  /**
   * Add a new language properties, which can resolve with getMessage() in the
   * configured language.
   *
   * @param language the language, which should append
   * @param properties the properties which will add in the language as
   * parameter
   */
   ...
   *
   * @param language the language, which should append
   * @param inputStream the properties which will add in the language as
   * parameter
   */
