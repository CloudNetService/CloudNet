...
public final class FileUtils {
    ...
    private static final Map<String, String> zipFileSystemProperties = Maps.newHashMap();
    ...
    {
        try (FileSystem fileSystem = FileSystems.newFileSystem(URI.create("jar:" + path.toUri()), zipFileSystemProperties))
        {
    ...
    public static void copy(InputStream inputStream, OutputStream outputStream, byte[] buffer, Consumer<Integer> lengthInputListener) throws IOException
    {
        ...
        {
            if (lengthInputListener != null) lengthInputListener.accept(len);
        ...
        if (from.isDirectory())
        {
            ...
            if (list != null && list.length > 0)
                for (File file : list)
                {
                    ...
                    if (file.isDirectory())
                        copyFilesToDirectory(file, new File(to.getAbsolutePath() + "/" + file.getName()));
                    else
                        copy(file.toPath(), Paths.get(to.getAbsolutePath() + "/" + file.getName()), buffer);
                }
        } else
            copy(from.toPath(), Paths.get(to.getAbsolutePath() + "/" + from.getName()), buffer);
    }
    ...
    {
        if (directories == null) return emptyZipByteArray();
            ...
            try (ZipOutputStream zipOutputStream = new ZipOutputStream(byteBuffer, StandardCharsets.UTF_8))
            {
                for (Path dir : directories)
                    if (Files.exists(dir))
                    {
                        ...
                        {
                            zipOutputStream.putNextEntry(new ZipEntry(dir.toFile().getName()));
