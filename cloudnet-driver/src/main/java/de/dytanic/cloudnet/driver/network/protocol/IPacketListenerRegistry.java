...
public interface IPacketListenerRegistry {
  ...
  /**
   * Returns the parent PacketListenerRegistry implementation instance if
   * exists
   */
  ...
  /**
   * Removes the listeners if they are registered on this listener registry. If
   * the listeners items are null, then it should ignored
   *
   * @param channel the channel, that the listener should listen on
  ...
  /**
   * Remove all listeners that are registered on the specific classLoader
   * instance
   *
   * @param classLoader the classLoader, from that all listeners that are
   * contained on the registry should remove
   */
  ...
  /**
   * Returns all channelIds that the packet listener registry has listeners
   * registered
   */
  ...
  /**
   * Handles an incoming packet and invoke all listeners that are registered in
   * this registry
   *
