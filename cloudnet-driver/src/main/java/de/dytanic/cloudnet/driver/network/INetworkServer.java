...
/**
 * The network server represents a server that can register and receive
 * INetworkClient connections and packets It is made for a simple read and write
 * network with a client and a server. You can bind this server on more than one
 * addresses
 *
 * @see INetworkClient
 */
public interface INetworkServer extends INetworkComponent, AutoCloseable {
   ...
   * @return true when the binding was successful or false if an error was threw
   * or the port is already bind
   */
