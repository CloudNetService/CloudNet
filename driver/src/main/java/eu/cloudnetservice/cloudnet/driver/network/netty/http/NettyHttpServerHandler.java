  ...
  private void handleMessage(@NonNull Channel channel, @NonNull HttpRequest httpRequest) {
    ...
    if (!context.cancelSendResponse) {
      ...
      if (response.bodyStream() != null) {
        ...
        // TODO: not yet migrated from netty4 to nett5
        future = channel
          .write(new DefaultHttpResponse(netty.protocolVersion(), netty.status(), netty.headers()))
          .flatMap($ -> channel.writeAndFlush(new HttpChunkedInput(/*new ChunkedStream(response.bodyStream())*/)));
      } else {
