...
public interface ModuleConfigStorage extends Named {
   ...
   */
  @Nullable
  Document loadConfig(@NonNull String configurationId);

  /**
   * @return
   */
  boolean configStoringSupported();

  /**
   * @param configurationId
   * @param document
   * @throws NullPointerException          if the given configuration id or configuration document is null.
   * @throws IllegalStateException         if storing of the given configuration failed for some reason.
   ...
   * @return
   */
  boolean configDeletionSupported();
