...
public class SpongeChatPlugin {
  ...
  public SpongeChatPlugin(@ConfigDir(sharedRoot = false) @NonNull Path configDirectory) {
    ...
    this.spongeAPIVersion = Sponge.pluginManager().plugin(Platform.API_ID)
  ...
  public void handle(@NonNull ConstructPluginEvent event) {
    ...
    try {
      ...
      if (Files.notExists(this.configFilePath)) {
        ...
        event.plugin().openResource(URI.create("config.conf")).ifPresent(in -> {
          try (in) {
            FileUtil.copy(in, this.configFilePath);
